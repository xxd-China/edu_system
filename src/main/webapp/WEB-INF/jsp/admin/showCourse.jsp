				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${pagebean.list}" var="item">
								<tr>
									...
									<td>${item.score}</td>
									<td>
										<button class="btn btn-default btn-xs btn-info" onClick="location.href='/admin/editCourse?courseid=${item.courseid}'">修改</button>
										<button class="btn btn-default btn-xs btn-danger btn-primary" onClick="location.href='/admin/removeCourse?courseid=${item.courseid}'">删除</button>
										<!--弹出框-->
									</td>
								</tr>
							</c:forEach>
							...
							<nav style="text-align: center">
								<ul class="pagination">
									...
									<c:if test="${pagebean.pagenum-2 > 0}">
										<li><a href="/admin/showCourse?page=${pagebean.pagenum-2}">${pagebean.pagenum-2}</a></li>
									</c:if>
									<c:if test="${pagebean.pagenum-1 > 0}">
										<li><a href="/admin/showCourse?page=${pagebean.pagenum-1}">${pagebean.pagenum-1}</a></li>
									</c:if>
									...
									<li><a href="/admin/showCourse?page=${pagebean.pagenum+1}">下一页&raquo;</a></li>
								</ul>
							</nav>
						</c:if>
				    </div>
